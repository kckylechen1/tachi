...
import type { BridgeConfig, MemoryCategory, MemoryEntry } from "./config.js";
...
export async function extractMemoryEntry(params: {
  ...
  try {
    ...
    let parsed = safeJsonParse<MemoryEntry>(text);
    if (!parsed) {
      ...
      if (recovered && recovered !== text) {
        ...
        parsed = safeJsonParse<MemoryEntry>(recovered);
      }
    }
    ...
    if (!parsed.category) parsed.category = "fact";
    ...
    if (err?.name === "AbortError") {
      throw new SigilError(
        "api_timeout",
        `extractor request timed out after ${config.extractor.timeoutMs}ms`,
      );
    }
...
function resolveMergeCategory(existingCategory: MemoryCategory, incomingCategory: MemoryCategory): MemoryCategory {
  if (existingCategory === "decision" || incomingCategory === "decision") return "decision";
  if (existingCategory === incomingCategory) return existingCategory;
  if (incomingCategory && incomingCategory !== "other") return incomingCategory;
  if (existingCategory) return existingCategory;
  return "other";
}

function getCategoryMergePrompt(category: string): string {
  switch (category) {
    case "preference":
...
export async function mergeMemoryEntries(params: {
  ...
  if (mergeCategory === "decision") {
    if (!incoming.metadata) incoming.metadata = { source_refs: [] };
